...
def fireCastModel(preProcessor, weightsFileName=None):
    if weightsFileName:
        #check if weightsFileName already has .h5 extension
        if weightsFileName.endswith('.h5'):
            fname = 'models/' + weightsFileName
        else:
    ...
    #model.compile(loss = tf.keras.losses.BinaryFocalCrossentropy(apply_class_balancing=True), 
