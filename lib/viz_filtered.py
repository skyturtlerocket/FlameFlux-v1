...
def compare_farsite(dataset, loc, res, size, fireDate):
    ...
    farsite_days = ['0711', '0712', '0713', '0714', '0715', '0716']
    farsite_wet = ['0711', '0712', '0713', '0714', '0715', '0716']
    # farsite_days = ['0716']
    # farsite_wet = ['0716']
    farsite_day_idx = 0
    next_day = False
...
def getNumbers(dataset, loc, res, size, fireDate):
    ...
    for burnName, date in dataset.getUsedBurnNamesAndDates():
        ...
        # print("low: ", low, " mid low: ", mid_low, " mid high: ", high_mid, " high: ", high)
        # continue
        # exit()
        # else:
        #     continue
        ...
        #handle case where no pixels were tested
        if len(posx) == 0:
            accCor = 0.0
            accIncor = 0.0
            print("ERROR: No pixels tested for this fire/date combination")
        else:
            try:
                accCor = float("%0.2f" % ((correct/(correct + incorrect)) * 100))
            except ZeroDivisionError:
                accCor = 0.0
                print("ERROR: Division by zero in accuracy calculation")
            accIncor = float("%0.2f" % ((incorrect/len(posx)) * 100))
