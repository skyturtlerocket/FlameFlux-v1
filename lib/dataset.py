...
class Dataset(object):
    ...
    def filterPoints(data, filterFunction):
        ...
        points = {}
        burns = list(data.burns.values())
        print(f"  Processing {len(burns)} fires for vulnerable pixels...")
        
        for i, b in enumerate(burns):
            print(f"    Fire {i+1}/{len(burns)}: {b.name} ({len(b.days)} dates)")
            dictOfDays = {}
            points[b.name] = dictOfDays
            days = list(b.days.values())
            
            for j, d in enumerate(days):
                if j % 5 == 0 or j == len(days) - 1:  # Show progress every 5 dates
                    print(f"      Date {j+1}/{len(days)}: {d.date}")
    ...
    def sample(self, goalNumber='max', sampleEvenly=True):
        ...
        if sampleEvenly:
            ...
            if goalNumber == 'max':
                ...
                #for non-even sampling with a goal number, we'll handle this in the loop
                samplesPerDay = None
        ...
        for i, day in enumerate(days):
            ...
            if sampleEvenly:
                ...
                if samplesPerDay == 'max':
                ...
                elif samplesPerDay is None:
                    #for non-even sampling with a goal number, distribute samples across remaining days
                    ...
                    nsamples = min(samplesPerDay, len(didBurn), len(didNotBurn))
                    didBurnSamples.extend(didBurn[:nsamples])
                    didNotBurnSamples.extend(didNotBurn[:nsamples])
    ...
    def vulnerablePixels(burn, day, radius=VULNERABLE_RADIUS):
        '''Return the indices of the pixels that are close to the current fire perimeter'''
        from scipy.ndimage import distance_transform_edt
        
        startingPerim = day.startingPerim
        
        #use Euclidean distance transform instead of morphological dilation
        #calculate distance from fire perimeter (invert so True = fire, False = non-fire)
        distance_from_fire = distance_transform_edt(~startingPerim.astype(bool))
        
        #find pixels within the specified radius
        vulnerable_mask = (distance_from_fire <= radius) & ~startingPerim.astype(bool)
        
        #get coordinates of vulnerable pixels
        ys, xs = np.where(vulnerable_mask)
